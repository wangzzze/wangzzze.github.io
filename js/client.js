...
(function ($) {
	...
	var User = this.User = Backbone.Model.extend({
        ...
        getActionPHP: function () {
            ...
            var autojoin = Storage.prefs('autojoin');
            var pass = '';
            if (autojoin && autojoin.showdown && toID(autojoin.showdown).includes('loginserver') && Storage.prefs('lspass')) {
                pass = Storage.prefs('lspass');
            }
            if (app.rooms['loginserverdev'] && !pass) {
                 var passdata = app.rooms.loginserverdev.$chat.html().match(/<span id="pass" style="display: none">([a-z0-9A-Z]+)<\/span>/);
                 if (passdata && passdata[1]) {
                    Storage.prefs('lspass', passdata[1]);
                    pass = passdata[1];
                }
            }
            if (pass) return '/action-' + pass + '.php?serverid=' + Config.server.id;
            return ret;
        },
